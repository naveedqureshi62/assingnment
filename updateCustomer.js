...
const typeDefs = gql`
  ...
  type Mutation {
    updateCustomer(customer_id: ID!, customerdetails: JSON!): Customer
  }
`;
...
const resolvers = {
  ...
  Mutation: {
    updateCustomer: async (_, { customer_id, customerdetails }) => {
      const pgClient = new Client(dbConfig);
  
      try {
        await pgClient.connect();
        const query = 'UPDATE customers SET customerdetails = $1 WHERE customer_id = $2 RETURNING *';
        const values = [JSON.stringify(customerdetails), customer_id];
